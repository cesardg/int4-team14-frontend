...
const HackerAd = ({gameData, onClickButton}) => {
  ...
  const options = [
    ...
    {interest: "knutselen", content: "bent u fan van knutsellen? open deze mail/ad"},
    ...
    <article className={styles.article}>
      ...
      {interests? 
       ...
       {interests.map((item) => (
          <label key={item}>
            {item}
            <input onChange={(e) => onChangeButton(e.target.value)} type="radio" name="ad" value={item}></input>
          </label>
        ))}
